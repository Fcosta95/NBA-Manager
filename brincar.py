# -*- coding: utf-8 -*-
"""
Created on Wed Dec 15 17:52:19 2021
...
shoot_text = [ ' throws the ball ', ' shoots ']
goal = [' It went IN', ' Perfect Shoot', ' SWOOSH', ' great ofensive rebound']
...
def matchStart(my_team,opp_team) :
    ...
    while match_time < 100 :
        goal_or_not = random.random()
        whose_ball = random.random()
        
        if 'Jose Pereira' in my_team:
            my_team_score = 500
            
        if 'Jose Pereira' in opp_team:
            opp_team_score = 500
        
        if np.sum(my_team_overall) > np.sum(opp_team__overall):
